...
static int xTaskCheckForTimeOut(unsigned int begin, unsigned int timeout)
{
	unsigned int mstime = us_ticker_read() / 1000L;
...
int get_button_event(int *button_event_type, int *button_event_count)
{
	...
	if (button_last_state != gpio_level) {
		...
		if ((gpio_level == BUTTON_GPIO_PRESSED)
				&& (xTaskCheckForTimeOut(button_timeout, long_press_tick)
						!= pdFALSE)) {
			...
			*button_event_count = 1;
			...
			return true;
				...
				&& (xTaskCheckForTimeOut(button_timeout, button_delay_tick)
						!= pdFALSE)) {
