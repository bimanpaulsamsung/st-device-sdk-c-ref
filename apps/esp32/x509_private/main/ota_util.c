...
#define CONFIG_FIRMWARE_VERSOIN_INFO_URL "http://org-fota-dn.ospserver.net/firmware/SKO/SM-V110S/version.test.xml"
#define CONFIG_FIRMWARE_UPGRADE_URL "http://fota-secure-dn.ospserver.net/firmware/SKO/SM-V110S/nspx/%s.bin?px-hash=%s&px-time=%s"
#define OTA_TEST_MODE 1
#else
...
#ifdef CONFIG_OTA_ALLOW_HTTP
#define FIRMWARE_URL_INFO_LEN 256
#define FIRMWARE_VERSION_LEN 20

static char g_firmware_url[FIRMWARE_URL_INFO_LEN] = {0,};
#endif
...
static bool _is_test_mode()
{
	return (OTA_TEST_MODE == 1)? true: false;
}
...
#if ! defined (CONFIG_OTA_ALLOW_HTTP)
	if (esp_http_client_get_transport_type(client) != HTTP_TRANSPORT_OVER_SSL) {
		printf("Transport is not over HTTPS\n");
		return ESP_FAIL;
	}
...
static ota_err_t _http_get_available_version(char *update_info, unsigned int update_info_len, char *firmware_version, char **new_version)
{
	ota_err_t ret = OTA_OK;
	...
	if (!update_info || !firmware_version) {
		printf("Invalid parameter \n");
		return OTA_ERR_INVALID_ARG;
	}
	...
	// Check if the device is updateable.
	char *upgrade_info = strstr(data, firmware_version);

	if (upgrade_info) {
		// parsing
		char *start_ptr = strstr(data, "<latest>");
		if (!start_ptr) {
			printf("Couldn't find a start attribute \n");
			return OTA_FAIL;
		}

		char *end_ptr = strstr(start_ptr, "</latest>");
		if (!end_ptr) {
			printf("Couldn't find a end attribute \n");
			return OTA_FAIL;
		}

		char firmware_url_info[FIRMWARE_URL_INFO_LEN + 1] = {0,};

		start_ptr += strlen("<latest>");

		int data_len = end_ptr - start_ptr;
		data_len = (data_len  >= FIRMWARE_URL_INFO_LEN)? FIRMWARE_URL_INFO_LEN: data_len;
		memcpy(firmware_url_info, start_ptr, data_len);

		printf("current version: %s, url : %s\n", firmware_version, firmware_url_info);

	    char *current_version = strtok(firmware_url_info, "/");
	    char *file_name = strtok(NULL, "/");
	    char *file_hash = strtok(NULL, "/");
	    char *session_time = strtok(NULL, "/");

		if (!current_version || !file_name || !file_hash || !session_time) {
			printf("Couldn't find version info\n");
			return OTA_FAIL;
		}

		snprintf(g_firmware_url, sizeof(g_firmware_url), CONFIG_FIRMWARE_UPGRADE_URL, file_name, file_hash, session_time);
		printf("url : %s\n", g_firmware_url);

		/* latest */
		str_len = strlen(current_version);
		latest_version = malloc(str_len + 1);
		if (!latest_version) {
			printf("Couldn't allocate memory to add latest version\n");
			ret = OTA_ERR_NO_MEM;
		...
		strncpy(latest_version, current_version, str_len);
		latest_version[str_len] = '\0';
...
static ota_err_t _http_update_device()
{
	printf("_http_update_device \n");
	ota_err_t ret = OTA_FAIL;

#ifdef CONFIG_OTA_ALLOW_HTTP

	char firmware_url[FIRMWARE_URL_INFO_LEN] = {0,};

	snprintf(firmware_url, sizeof(firmware_url), "%s", g_firmware_url);
...
void ota_check_for_update(void *user_data)
{
    ...
    if (ret == OTA_OK) {
        ...
        if (_is_test_mode() == true)
            ret = _http_get_available_version(read_data, read_data_len, current_version, &available_version);
        else
