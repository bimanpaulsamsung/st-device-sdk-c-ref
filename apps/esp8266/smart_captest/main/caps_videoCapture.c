...
static void caps_videoCapture_cmd_capture_cb(IOT_CAP_HANDLE *handle,
...
static void caps_videoCapture_init_cb(IOT_CAP_HANDLE *handle, void *usr_data)
{
    caps_videoCapture_data_t *caps_data = usr_data;
    if (caps_data && caps_data->init_usr_cb)
        caps_data->init_usr_cb(caps_data);
    caps_videoCapture_attr_clip_send(caps_data);
    caps_videoCapture_attr_stream_send(caps_data);
}

caps_videoCapture_data_t *caps_videoCapture_initialize(IOT_CTX *ctx, const char *component, void *init_usr_cb, void *usr_data)
{
    caps_videoCapture_data_t *caps_data = NULL;
    int err;

    caps_data = malloc(sizeof(caps_videoCapture_data_t));
    if (!caps_data) {
        printf("fail to malloc for caps_videoCapture_data\n");
        return NULL;
    }

    memset(caps_data, 0, sizeof(caps_videoCapture_data_t));

    caps_data->init_usr_cb = init_usr_cb;
    caps_data->usr_data = usr_data;

    caps_data->get_clip_value = caps_videoCapture_get_clip_value;
    caps_data->set_clip_value = caps_videoCapture_set_clip_value;
    caps_data->attr_clip_send = caps_videoCapture_attr_clip_send;
    caps_data->get_stream_value = caps_videoCapture_get_stream_value;
    caps_data->set_stream_value = caps_videoCapture_set_stream_value;
    caps_data->attr_stream_send = caps_videoCapture_attr_stream_send;
    if (ctx) {
        caps_data->handle = st_cap_handle_init(ctx, component, caps_helper_videoCapture.id, caps_videoCapture_init_cb, caps_data);
    }
    if (caps_data->handle) {
        err = st_cap_cmd_set_cb(caps_data->handle, caps_helper_videoCapture.cmd_capture.name, caps_videoCapture_cmd_capture_cb, caps_data);
        if (err) {
            printf("fail to set cmd_cb for capture of videoCapture\n");
    }
    } else {
        printf("fail to init videoCapture handle\n");
    }
