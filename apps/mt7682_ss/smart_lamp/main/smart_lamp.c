...
#define GREEN_LED_IDX	1
#define BLUE_LED_IDX	2
...
{
	if (state == 0) {
		device_led_control(RED_LED_IDX, 0);
		device_led_control(GREEN_LED_IDX, 0);
		device_led_control(BLUE_LED_IDX, 0);
	} else {
		device_led_control(RED_LED_IDX, (smartlamp_color_red > 127));
		device_led_control(GREEN_LED_IDX, (smartlamp_color_green > 127));
		device_led_control(BLUE_LED_IDX, (smartlamp_color_blue > 127));
	}
}
			...
			iot_cap_cmd_data_t *cmd_data, void *usr_data)
{
	...
	IOT_INFO("HSL (%lf, %lf, %d), RGB (%d, %d, %d)",
...
void app_main(void)
{
	...
	start_level_timer(1, (void*)level_timer_callback);
	...
	//	implement callback function
		...
		if (iot_err)
			IOT_ERROR("fail to set cmd_cb for setcolor");
