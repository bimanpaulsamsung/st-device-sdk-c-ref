...
void change_switch_state(int switch_state)
{
    if (switch_state == SWITCH_OFF) {
        XMC_GPIO_SetOutputLow(GPIO_OUTPUT_MAINLED_PORT, GPIO_OUTPUT_MAINLED);
    } else {
        XMC_GPIO_SetOutputHigh(GPIO_OUTPUT_MAINLED_PORT, GPIO_OUTPUT_MAINLED);
    }
}
...
int get_button_event(int* button_event_type, int* button_event_count)
{
    ...
    gpio_level = XMC_GPIO_GetInput(GPIO_INPUT_BUTTON_PORT, GPIO_INPUT_BUTTON);
...
void iot_gpio_init(void)
{
    ...
    XMC_GPIO_HWCTRL_t gpio_hwctrl;

    /* Button */
    gpio_config.mode = XMC_GPIO_MODE_INPUT_PULL_UP;
    gpio_hwctrl = XMC_GPIO_HWCTRL_DISABLED;
    XMC_GPIO_Init(GPIO_INPUT_BUTTON_PORT, GPIO_INPUT_BUTTON, &gpio_config);
    XMC_GPIO_SetHardwareControl(GPIO_INPUT_BUTTON_PORT, GPIO_INPUT_BUTTON, gpio_hwctrl);

    /* LED */
    gpio_config.mode = XMC_GPIO_MODE_OUTPUT_PUSH_PULL;
    gpio_config.output_level = XMC_GPIO_OUTPUT_LEVEL_LOW;
    gpio_hwctrl = XMC_GPIO_HWCTRL_DISABLED;
    XMC_GPIO_Init(GPIO_OUTPUT_MAINLED_PORT, GPIO_OUTPUT_MAINLED, &gpio_config);
    XMC_GPIO_SetHardwareControl(GPIO_OUTPUT_MAINLED_PORT, GPIO_OUTPUT_MAINLED, gpio_hwctrl);
