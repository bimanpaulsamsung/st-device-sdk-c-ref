static double caps_$CAPS_ID$_get_$ATTR_NAME$_value(caps_$CAPS_ID$_data_t *caps_data)
{
    if (!caps_data) {
...
static void caps_$CAPS_ID$_set_$ATTR_NAME$_value(caps_$CAPS_ID$_data_t *caps_data, double value)
{
    if (!caps_data) {
        printf("caps_data is NULL\n");
        return;
...
static void caps_$CAPS_ID$_attr_$ATTR_NAME$_send(caps_$CAPS_ID$_data_t *caps_data)
{
    ...
    cap_evt = st_cap_attr_create_number((char *) caps_helper_$CAPS_ID$.attr_$ATTR_NAME$.name, caps_data->$ATTR_NAME$_value, caps_data->$ATTR_NAME$_unit);
    if (!cap_evt) {
